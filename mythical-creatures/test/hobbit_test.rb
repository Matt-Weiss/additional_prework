...
class HobbitTest < Minitest::Test
  ...
  def test_is_old_at_age_of_101
    ...
    hobbit = Hobbit.new('Baern')# create a hobbit
    101.times do
      hobbit.celebrate_birthday# have hobbit age 101 years
    end
  ...
  def test_hobbit_has_the_ring_if_its_name_is_frodo
    ...
    ringbearer = Hobbit.new('Frodo')# create a hobbit named Frodo
    gardner = Hobbit.new('Sam')# create a second hobbit named Sam
    assert ringbearer.has_ring?# check that .has_ring? for Frodo returns true
    refute gardner.has_ring?# check that .has_ring? for Sam returns false
  end

  def test_hobbits_are_short

    hobbit = Hobbit.new('Bilbo')# create a hobbit
