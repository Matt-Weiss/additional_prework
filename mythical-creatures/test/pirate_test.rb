...
class PirateTest < Minitest::Test
  ...
  def test_a_pirate_has_booty
    ...
    assert_equal 0, pirate.booty# check that the pirate starts with 0 booty
  end

  def test_a_pirate_gets_100_booty_for_robbing_ships

    pirate = Pirate.new('Jack')# create a pirate
      5.times do pirate.commit_piracy
      end# rob some ships
    assert_equal 500, pirate.booty# check that the pirate got 100 booty for each ship it robbed
  end
